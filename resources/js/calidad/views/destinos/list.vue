   ...
   <div class="container-fluid px-0 mx-0">
      ...
      <b-card no-body class="mb-0">
            ...
            <b-row>
               ...
               <b-col md="6">
                  ...
                  <b-input-group size="sm">
                     ...
                     <template #append v-if="$can('write', 'destinos')">
                        <b-button variant="primary" :to="{ name: 'create.destino' }">
         ...
         <b-table ref="refTable" :items="fetchData" responsive :fields="tableColumns" primary-key="id"
            :sort-by.sync="sortBy" empty-text="No se encontró ninguna destino" :sort-desc.sync="isSortDirDesc"
            sticky-header="700px" :no-border-collapse="false" small :busy="loading" show-empty>

            <template #empty>
               <span class="text-nowrap text-center d-block">
                  No se encontró níngún destino...
               </span>
            </template>
            ...
            <template #cell(compania)="{item}">
               <span class="text-nowrap">
                  {{ item.compania? item.compania.nombre : 'Sin definir' }}
               </span>
            </template>
           ...
           <!-- Column: Actions -->
            <template #cell(actions)="{ item }">
               ...
               <b-button-group size="sm">
                  ...
                  <b-button variant="primary" title="Editar" :to="{ name: 'edit.destino', params: { id: item.id } }"
                     v-if="$can('update', 'destinos')">
                     <feather-icon icon="EditIcon" />
                  </b-button>

                  <b-button variant="danger" @click="eliminar(item.id)" title="Eliminar destino"
                     v-if="$can('delete', 'destinos')">
...
import useListDestino from './useListDestino';

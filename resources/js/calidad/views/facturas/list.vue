   ...
   <div class="container-fluid px-0 mx-0">
      ...
      <b-card no-body class="mb-0">
            ...
            <b-row>
               ...
               <b-col md="6">
                  ...
                  <b-input-group size="sm">
                     ...
                     <template #append v-if="$can('write', 'facturas')">
                        <b-button variant="primary" :to="{ name: 'create.factura' }">
         ...
         <b-table ref="refTable" :items="fetchData" responsive :fields="tableColumns" primary-key="id"
            :sort-by.sync="sortBy" empty-text="No se encontró ninguna factura" :sort-desc.sync="isSortDirDesc"
            sticky-header="700px" :no-border-collapse="false" small :busy="loading" show-empty>

            <template #empty>
               <span class="text-nowrap text-center d-block">
                  No se encontró nínguna factura...
               </span>
            </template>
            ...
            <template #table-busy>
             ...
             <template #cell(fecha)="{item}">
               <span class="text-nowrap">
                  {{ item.fecha | fecha('D/MM/YYYY') }}
               </span>
            </template>

             <template #cell(created_at)="{item}">
               <span class="text-nowrap">
                  {{ item.created_at | fecha('D/MM/YYYY h:m a') }}
               </span>
            </template>



            <template #cell(compania)="{item}">
               <span class="text-nowrap">
                  {{ item.compania.nombre }}
               </span>
            </template>

            <template #cell(conduce)="{item}">
               <span class="text-nowrap">
                  {{ item.orden ? item.orden.numero : 'Sin definir' }}
               </span>
            </template>
            ...
            <template #cell(actions)="{ item }">
               ...
               <b-button-group size="sm">
                  ...
                  <b-button variant="primary" title="Ver" :to="{ name: 'show.factura', params: { id: item.id } }"
                     v-if="$can('read', 'facturas')">
                     <feather-icon icon="EyeIcon" />
                  </b-button>

                  <b-button variant="danger" @click="eliminar(item.id)" title="Eliminar factura"
                     v-if="$can('delete', 'facturas')">
...
import useListFacturas from './useListFacturas';

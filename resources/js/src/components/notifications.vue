  ...
  <div>
    ...
    <b-nav-item-dropdown class="dropdown-notification mr-25" menu-class="dropdown-menu-media" lazy right
        ...
        <div class="dropdown-header pb-1 pt-0" v-if="sinleer.length > 0">
          ...
          <b-button-group size="sm">
            <b-button variant="outline-dark" @click="todoleido()" size="sm"
              v-b-tooltip.hover.top="'Marcar todo como leido'">
              Marcar todos como leido
            </b-button>

            <b-button variant="outline-dark" v-if="formulario.notificaciones.filter(val => val.seleccionado).length > 0"
              @click="marcarLeido()" v-b-tooltip.hover.top="'Marcar como leidos'">
              <feather-icon icon="EyeIcon" size="12" />
            </b-button>

            <b-button variant="outline-dark" v-if="formulario.notificaciones.filter(val => val.seleccionado).length > 0"
              @click="eliminarSeleccionadas()" v-b-tooltip.hover.top="'Eliminar seleccionadas'">
      ...
      <b-tabs content-class="mt-1" fill pills end>
        <b-tab>
          <template #title>
            <strong class="mr-1 text-dark">Sin leer </strong>
          ...
          <section class="content-notificacion">
            ...
            <b-form-checkbox size="md" inline v-for="(notification,i) in formulario.notificaciones" :key="i"
              v-model="notification.seleccionado" class="mt-1 ml-2 mr-1">
              <b-link>
                <b-media @click="verNotificacion(notification)" class="notificacion-list">
                  <template #aside v-if="notification.data">
                    <b-avatar v-if="notification.data.usuario" size="32" :src="notification.data.avatar"
                      :text="notification.data.usuario" :variant="notification.data.type" />
        ...
        <b-tab title="Leidos" title-item-class="text-dark">

          <template #title>
            <strong class="mr-1 text-dark">Leidos </strong>
            <b-badge pill variant="primary">
              {{leidas.length}}
            </b-badge>
          </template>
                ...
                <section class="row">
                  ...
                  <div class="col-12 col-md-4">
                    <b-button-group size="sm" class="btns">
                      <b-button variant="danger" v-b-tooltip.hover.top="'Eliminar Notificación'"
                        @click.stop="eliminarNotification(notification.id)">
                        <feather-icon icon="Trash2Icon" size="12" />
                      </b-button>

                      <b-button variant="dark" v-b-tooltip.hover.top="'Marcar como no leida'"
    ...
    <b-modal id="modalNotification" content-class="shadow" no-close-on-esc :title="notification.data.titulo"
      no-close-on-backdrop ref="modalNotificacion" @hide="cerrarNotification" centered size="lg">
      ...
      <template #modal-footer>
        <div class="btn-group d-flex justify-content-start">
          ...
          <b-button @click="irNotification(notification.data.url)" v-if="notification.data.btn" variant="primary"
            size="sm">
